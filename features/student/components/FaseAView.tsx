...
export function FaseAView({ classroomData }: FaseAViewProps) {
  ...
  const isLessonUnlocked = (lessons: typeof activeLessons, lIdx: number): boolean => {
    ...
    <div className="w-full space-y-3.5 select-none font-sans">
      ...
      {focusedLesson ? (
        ...
        <div className="space-y-3.5">
          ...
          {/* Daftar Bab (Accordion List ala Ruangguru) */}
          <section className="space-y-2.5">
            {activeModules.length === 0 ? (
              <div className="p-8 text-center bg-white rounded-2xl border border-slate-100">
                <p className="text-xs font-bold text-slate-400">Belum ada bab aktif.</p>
              </div>
            ) : (
              activeModules.map((mod, mIdx) => {
                ...
                return (
                  <div
                    key={mod.id}
                    className="bg-white rounded-2xl border border-slate-100 shadow-xs hover:border-teal-300 transition-all overflow-hidden"
                  >
                    {/* Klik Bab langsung membuka halaman materi khusus di /siswa/kelas/[id]/bab/[moduleId] */}
                    {isUnlocked ? (
                      <Link
                        href={`/siswa/kelas/${classroomData.classData.id}/bab/${mod.id}`}
                        className="w-full p-3.5 flex items-center justify-between gap-3 text-left hover:bg-slate-50/80 active:scale-98 transition-all cursor-pointer group"
                      >
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div
                            className={`w-9 h-9 rounded-xl flex items-center justify-center font-black text-xs shrink-0 ${
                              isCompleted
                                ...
                                : 'bg-teal-600 text-white shadow-2xs group-hover:bg-teal-700'
                            }`}
                          >
                            {isCompleted ? (
                          ...
                          <div className="min-w-0 flex-1 py-0.5">
                            ...
                            <h4 className="text-xs sm:text-sm font-black text-slate-900 leading-snug break-words group-hover:text-teal-700 transition-colors">
                        ...
                        <div className="flex items-center gap-2 shrink-0 self-center">
                          ...
                          <div className="w-7 h-7 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-teal-600 group-hover:text-white transition-colors">
                      ...
                      <button
                        ...
                        className="w-full p-3.5 flex items-center justify-between gap-3 text-left bg-slate-50/90 opacity-60 cursor-not-allowed"
                      >
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div className="w-9 h-9 rounded-xl bg-slate-200 text-slate-400 flex items-center justify-center font-black text-xs shrink-0">
                          ...
                          <div className="min-w-0 flex-1 py-0.5">
                            ...
                            <h4 className="text-xs sm:text-sm font-black text-slate-500 leading-snug break-words">
                              {cleanModuleTitle(mod.title)}
                            </h4>
                          </div>
                        </div>
