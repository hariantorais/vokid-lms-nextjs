      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3.5">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <span
              className={`inline-flex items-center gap-1 text-[10px] font-black uppercase px-2 py-0.5 rounded-md ${activeLesson.content_type === 'VIDEO'
        ...
        {/* Homeschool Acceleration Action Toolbar */}
        <div className="flex flex-wrap items-center gap-2 pt-1 pb-1">
          <Link
            href={`/guru/meja-ajar/${activeLesson.id}`}
            className="h-9 px-3.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 active:scale-95 text-slate-950 font-black text-xs flex items-center gap-1.5 shadow-sm transition-all"
          >
            <Clock className="w-3.5 h-3.5" />
            <span>Meja Ajar (Sesi 70m)</span>
          </Link>

          <Link
            href={`/guru/pelajaran/${activeLesson.id}/cetak`}
            target="_blank"
            className="h-9 px-3.5 rounded-xl bg-slate-100 hover:bg-slate-200 active:scale-95 text-slate-800 font-bold text-xs flex items-center gap-1.5 border border-slate-200/80 transition-all"
          >
        ...
        {activeLesson.learning_objectives ? (
          <div className="p-3.5 rounded-xl bg-sky-50/80 border border-sky-200/80 text-sky-950 space-y-1">
            ...
            <div className="pl-5">
              <MarkdownContent content={activeLesson.learning_objectives} size="xs" />
            </div>
          </div>
        ) : (
        ...
        {activeLesson.content_type === 'TEXT' ? (
          <div className="bg-slate-50/80 p-3.5 rounded-xl border border-slate-200/80">
            {activeLesson.content_text ? (
              <MarkdownContent content={activeLesson.content_text} size="xs" />
            ) : (
              <span className="text-xs italic text-slate-400">Belum ada isi teks modul.</span>
            )}
          </div>
            ...
            {assignments.map((asg) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span
                      className={`px-2 py-0.5 rounded-md text-[10px] font-extrabold flex items-center gap-1 ${asg.type === 'QUIZ_CBT'
